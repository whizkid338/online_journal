...
def find(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            entries = entrySearch(request.user.username)
    else:
        form = FindForm()

    # process username here from query
    # entries = entrySearch("testUser")
    #return render_to_response('find.html', {'results': results, 'tags': tags, 'entries': entries, 'form': form}, RequestContext(request))
    return render_to_response('find.html', {'entries': entries, 'form': form}, RequestContext(request))
